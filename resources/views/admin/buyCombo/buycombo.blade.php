                            ...
                            <div id="Combos" class="mt-5 collapse show" data-bs-parent="#mainTicket">
                                ...
                                <div class="row g-2 mt-2 row-cols-2" data-bs-parent="#mainContent">
                                                    ...
                                                    <div class="col-lg-4 col-12">
                                                            ...
                                                            @if(strstr($combo['image'],"https") == "")
                                                                <img style="width: 200px; height:200px"
                                                                     ...
                                                                     alt="user1">
                                                            @else
                                                                <img style="width: 200px;heigh:200px"
                                                    ...
                                                    <div class="col-lg-8 col-12">
                                                        ...
                                                        <div class="card-body">
                                                            <div class="input-group">
                                                                ...
                                                                <input id="soluongcombo_{{$combo->id_combo}}" type="number" class="form-control input_combo ps-3"
                                    ...
                                    @foreach($foods as $food)
                                            <!-- Foods -->
                                            <div class="col">
                                                <div class="card px-0 overflow-hidden" id="Food_{{$food->id_food}}"
                                                     style="background: #f5f5f5">
                                                    <div class="row g-0">
                                                        <div class="col-lg-4 col-12">
                                                           
                                                                <img class="img-fluid w-100" alt="..." style="max-height: 361px; max-width: 241px"
                                                                     src="{{ asset('Images/food/'. $food['hinh_food']) }}">
                                                            
                                                        </div>
                                                        <div class="col-lg-8 col-12">
                                                            <div class="card-body">
                                                                <h5 class="card-title text-dark">{{ $food->ten_food }}</h5>
                                                                <p class="card-text">Giá: <span class="fw-bold">{{ number_format($food->gia_food) }} đ</span></p>
                                                            </div>
                                                            <div class="card-body">
                                                                <div class="input-group">
                                                                    <button class="btn mb-0 minus_food disabled"
                                                                            onclick="minusFood({{$food->id_food}}, {{$food->gia_food}}, '{{ $food->ten_food }}')">
                                                                        <i class="fa-solid fa-circle-minus"></i>
                                                                    </button>
                                                                    <input id="soluongfood_{{ $food->id_food }}" type="number" class="form-control input_food ps-3"
                                                                           name="food[{{$food->id_food}}]"
                                                                           value="0" readonly min="0"
                                                                           style="max-width: 80px" aria-label="">
                                                                    <button class="btn mb-0 plus_food"
                                                                            onclick="plusFood({{$food->id_food}}, {{$food->gia_food}}, '{{ $food->ten_food }}')">
                                                                        <i class="fa-solid fa-circle-plus"></i>
                                                                    </button>
                                                                </div>
                                                            </div>
                                    ...
                                    <div class="bg-dark-subtle p-5">
                                        <div class="row row-cols-1" data-bs-parent="#mainContent">
                                            <div class="col">
                                                ...
                                                <input type="hidden" name="ticketSeat_totalPrice" id="ticketSeat_totalPrice_input" value="">
...
@section('scripts')
            ...
            function plusCombo(id_combo, gia, ten_combo) {
                $iCombo++;
                if ($iCombo !== 0) {
                    $('.btn_next').removeClass('disabled');
                }
                let $inputCombo = $('#soluongcombo_' + id_combo);
                console.log('Số lượng hiện tại:', $inputCombo.val());
                ...
                if (parseInt($inputCombo.val()) === 1) {
                    $('#ticket_combos').append(`<p id="ticketCombo_${id_combo}">${ten_combo} x ${parseInt($inputCombo.val())}</p>`);
                    console.log('Ticket Combo đã được thêm:', ten_combo, 'x', parseInt($inputCombo.val()));
                } else {
                    $(`#ticketCombo_${id_combo}`).replaceWith(`<p id="ticketCombo_${id_combo}">${ten_combo} x ${parseInt($inputCombo.val())}</p>`);
                    console.log('Ticket Combo đã được thay thế:', ten_combo, 'x', parseInt($inputCombo.val()));
                }
                $sum += gia;
                console.log('Tổng giá:', $sum.toLocaleString('vi-VN'));
                $('#ticketSeat_totalPrice').text($sum.toLocaleString('vi-VN'));
                $ticket_combos[id_combo] = [id_combo, parseInt($inputCombo.val())];
                console.log('Mảng ticket combos:', $ticket_combos);
                $('#ticketSeat_totalPrice_input').val($sum.toLocaleString('vi-VN'));

            }

            function minusCombo(id_combo, gia, ten_combo) {
                let $inputCombo = $('#soluongcombo_' + id_combo);
                if ($inputCombo.val() === '0') {
            ...
            plusFood = (id_food, gia_food, ten_food) => {
                $iCombo++;
                if($iCombo !== 0) {
                    $('.btn_next').removeClass('disabled');
                }
               let $inputFood = $('#soluongfood_'+id_food);
                $inputFood.val(parseInt($inputFood.val()) + 1);
                $inputFood.parent().find('.minus_food').removeClass('disabled');
                if (parseInt($inputFood.val()) === 1) {
                    $('.minus_food_' + id_food).removeClass('disabled');
                    $('#ticket_combos').append(`<p id="ticketFood_${id_food}">${ten_food} x ${parseInt($inputFood.val())}</p>`);
                } else {
                    $(`#ticketFood_${id_food}`).replaceWith(`<p id="ticketFood_${id_food}">${ten_food} x ${parseInt($inputFood.val())}</p>`);
                }
                $sum += gia_food;
                $('#ticketSeat_totalPrice').text($sum.toLocaleString('vi-VN'));
                $ticket_foods[id_food] = [id_food, parseInt($inputFood.val())];
                $('#ticketSeat_totalPrice_input').val($sum.toLocaleString('vi-VN'));

            }

            minusFood = (id_food, gia_food, ten_food) => {
                let $inputFood = $('#soluongfood_'+id_food);
                $iCombo--;
                if($iCombo === 0) {
                    $('.btn_next').addClass('disabled');
                ...
                if (parseInt($inputFood.val()) === 0) {
                    $('.minus_food_' + id_food).addClass('disabled');
                    $(`#ticketFood_${id_food}`).remove();
                } else {
                    $(`#ticketFood_${id_food}`).replaceWith(`<p id="ticketFood_${id_food}">${ten_food} x ${parseInt($inputFood.val())}</p>`);
                }
                $sum -= gia_food;
