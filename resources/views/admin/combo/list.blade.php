                                    ...
                                    @foreach($combos as $combo)
                                        <tr>
                                            <td class="align-middle text-center">
                                                <h6 class="mb-0 text-sm ">{{ $combo->ten_combo }}</h6>
                                            </td>
                                            <td class="align-middle text-center">
                                                ...
                                                @else
                                                    <img style="width: 300px"
                                                         src="{!! $combo->hinh !!}" alt="user1">
                                                @endif
                                            </td>
                                            <td class="align-middle text-center">
                                                @foreach($combo->foods as $food)
                                                    {{ $food->ten_food . ' x '. $food->pivot->so_luong}} <br>
                                                @endforeach
                                            </td>
                                            <td class="align-middle text-center">
                                                <span class="text-secondary font-weight-bold">{{ number_format($combo->gia) }} đ</span>
                                            </td>
                                            <td id="status{{$combo->id}}" class="align-middle text-center text-sm ">
                                                {{-- @if($combo->status == 1)
                                                    <a href="javascript:void(0)" class="btn_active" onclick="changeStatus({{ $combo->id_combo}},0)">
                                                        <span class="badge badge-sm bg-gradient-success">Online</span>
                                                    </a>
                                                @else
                                                    <a href="javascript:void(0)" class="btn_active" onclick="changeStatus({{ $combo->id_combo }},1)">
                                                        <span class="badge badge-sm bg-gradient-secondary">Offline</span>
                                                    </a>
                                                @endif --}}

                                                <form action="{{ route('changeStatusCombo') }}" method="POST">
                                                    @csrf
                                                    <input type="hidden" name="combo_id" value="{{ $combo->id_combo }}">
                                                    @if($combo->status == 1)
                                            ...
                                            <td class="align-middle">
                                                ...
                                                <a href="{{ url('admin/combo/edit', $combo->id_combo ) }}" class="text-secondary font-weight-bold text-xs" data-toggle="tooltip"
                                                data-original-title="Edit combo" data-bs-target="#comboEdit_{{$combo->id_combo}}"
                                            ...
                                            <td class="align-middle">
                                                <form action="{{ url('admin/combo/delete',$combo->id_combo ) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
