                ...
                <div class="modal-body">
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-12 food_group" id="food_group">
                                ...
                                <div class="input-group m-1">
                                    ...
                                    <button type="button" class="btn btn-danger mb-0 delete_food">DELETE</button>
                                </div>
                            </div>
                            <button type="button" class="btn m-1 btn-primary add_food" id="add_food_button">ADD FOOD</button>
                        </div>
                    </div>
                </div>
...
document.getElementById('add_food_button').addEventListener('click', function() {
    const foodGroup = document.getElementById('food_group');
    const newFoodInputGroup = document.createElement('div');
    newFoodInputGroup.classList.add('input-group', 'm-1');
    newFoodInputGroup.innerHTML = `
        <span class="input-group-text text-black-50">@lang('lang.food'): </span>
        <select type='text' name='food[]' class="form-select" aria-label="food">
            @foreach($foods as $food)
                <option value="{{$food->id_food}}">{{$food->ten_food}}</option>
    ...
    foodGroup.appendChild(newFoodInputGroup);

    // Add event listener for delete button
    newFoodInputGroup.querySelector('.delete_food').addEventListener('click', function() {
        newFoodInputGroup.remove();
    });
});
