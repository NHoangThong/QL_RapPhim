...
class PaymentController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        $vnp_TxnRef = $ticket->ma_code; //Mã giao dịch thanh toán tham chiếu của merchant
        $vnp_Amount = $ticket->tong_tien_ve; // Số tiền thanh toán
...
//         ]
        ...
        $ticket = Ve::where('ma_code', $request->vnp_TxnRef)->get()->first();

        switch ($request->vnp_ResponseCode) {
            case '00':
                
                if ($request->hasDiscount != 'false') {
                    $ticket->trang_thai_giam_gia = true;
                }
                $ticket->trang_thai_dat_ve=0;
                ...
                {
                    $money_payment += $ticket['tong_tien_ve'];
            }
                if($money_payment < 4000000){
                    $point = ($ticket['tong_tien_ve']) * 5 / 100;
                }else{
                    $point = ($ticket['tong_tien_ve']) * 10 / 100;
                }
                if ($request->hasDiscount == 'false') {
                    $user->point += $point;
                } else {
                    $discount  = Khuyenmai::find($request->hasDiscount);
                    $discount->so_luong--;
                    $discount->save();
                }

                $user->save();
                return redirect()->action([WebController::class, 'ticketCompleted'], $ticket->id_ve);
            default:
                Ve::where('ma_code', $request->vnp_TxnRef)->delete();
                return redirect('/');
        }
    }
}
